    ...
    <div id="commentSection" class="max-w-xs overflow-y-auto border p-3 max-h-[524px]" ref="commentSectionRef">
      <ul>
        <li v-for="msg in pinMessageList" :key="msg.id">
          <div v-if="chatList.find((chat) => chat.id == msg.messageId)">{{ userList.filter((user) => user.id == chatList.find((chat) => chat.id == msg.id).senderId)[0].name + "さん: " + chatList.filter((chat) => chat.id == msg.messageId)[0].content }}
          </div>
        </li>
      </ul>
    </div>
